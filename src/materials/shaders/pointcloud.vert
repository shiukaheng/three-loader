...
void main() {
	float displaceFac = 0.01;
	float displaceSize = 8.0;
	float timeFac = 1.0;
	
	vec4 mvPosition_ = modelViewMatrix * vec4(position, 1.0);
	vec4 mPosition = modelMatrix * vec4(position, 1.0);

	float displaceX = snoise(vec4(mPosition.x*displaceSize, mPosition.y*displaceSize, mPosition.z*displaceSize, 0.0+timePassed*timeFac));
	float displaceY = snoise(vec4(mPosition.x*displaceSize, mPosition.y*displaceSize, mPosition.z*displaceSize, 10.0+timePassed*timeFac));
	float displaceZ = snoise(vec4(mPosition.x*displaceSize, mPosition.y*displaceSize, mPosition.z*displaceSize, 20.0+timePassed*timeFac));
